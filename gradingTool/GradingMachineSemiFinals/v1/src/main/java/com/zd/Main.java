...
public class Main {
    ...
    private static int[][] diskPoint = new int[MAX_DISK_NUM][Disk_Head_Num];
    static int[][] diskToken = new int[MAX_DISK_NUM][Disk_Head_Num];
    static char[][] diskLastAction = new char[MAX_DISK_NUM][Disk_Head_Num];
    static int[][] preToken = new int[MAX_DISK_NUM][Disk_Head_Num];
    ...
    static int firstScoreCursor = 1;
    static int secondScoreCursor = 1;

    public static void main(String[] args) {
        //读取sample_practise.in文件并通过UDP输出
        String file_path = "src/main/resources/sample_practice.in";
        ...
        String send_data = lines.get(0);//发送T M N V G K
        ...
        long startTime;
        long endTime;
        List<Request> unProcessRequests = new ArrayList<>();
        for(int timestamp=1;timestamp<=T+EXTRA_TIME;timestamp++){
            ...
            while(firstScoreCursor<requests.size() && timestamp-requests.get(firstScoreCursor).startTimeStamp >= 105){
                ...
                if(!requests.get(firstScoreCursor).isAborted && !requests.get(firstScoreCursor).isDone && !requests.get(firstScoreCursor).isBusy){
                    ...
                    unProcessRequests.add(requests.get(firstScoreCursor));
                }
            ...
            while(secondScoreCursor<requests.size() && timestamp-requests.get(secondScoreCursor).startTimeStamp >= 10){
                ...
                if(!requests.get(secondScoreCursor).isAborted && !requests.get(secondScoreCursor).isDone && !requests.get(firstScoreCursor).isBusy){
            ...
            if(del_abort_num>0){
                ...
                for(int j=1;j<=del_abort_num;j++){
                    ...
                    abortRequestsNum += 1;
                    unProcessRequests.remove(requests.get(requestId));
                }
            ...
            if(disk_action.length != Disk_Head_Num *N){
                System.out.println("选手输入出错：磁头指令数量与磁盘数量不一致！");
                System.exit(1);
            }

            for(int j=1;j<=N;j++){
                for(int k = 0; k< Disk_Head_Num; k++){
                    int token = G, index=0;
                    while(index<disk_action[(j-1)*Disk_Head_Num+k].length() && token>0){
                        char action = disk_action[(j-1)*Disk_Head_Num+k].charAt(index);
                        if(action == 'p'){
                            if(token>=1){
                                token--;
                                diskToken[j][k] = token;
                                diskPoint[j][k] = diskPoint[j][k]%V+1;
                                diskLastAction[j][k] = 'p';
                                preToken[j][k] = 1;

                            }else{
                                String message = String.format("磁盘%d的动作字符串%s中第%d个动作失效，原因为令牌不足！",j,disk_action[(j-1)*Disk_Head_Num+k],index+1);
                                System.out.println(message);
                                break;
                            }
                            index++;
                            ...
                            int consume;
                            if(diskLastAction[j][k] != 'r'){
                                consume = 64;
                            }else{
                                consume = (int) Math.max(16,Math.ceil(preToken[j][k]*0.8));
                             }
                            if(token>=consume){
                                token -= consume;
                                //判断磁盘中被读取的对象
                                int objectId = disk[j][diskPoint[j][k]];
                                if(objectId == 0){//判断读取内容是否为空，objectId=0则为空
                                    String message = String.format("选手输入出错：磁盘%d读取存储位置%d时，磁盘内容为空！",j,diskPoint[j][k]);
                                    System.out.println(message);
                                }else{
                                    int objectBlockIndex = getObjectBlockIndex(objectId,j,diskPoint[j][k]);
                                    int currentId = storageObjects.get(objectId).lastRequestPoint;
                                    int prevId = requests.get(currentId).prevId;
                                    boolean isRequestValid;
                                    while(currentId != 0){
                                        isRequestValid = (!requests.get(currentId).isAborted && requests.get(currentId).isGenerated && !requests.get(currentId).isDone);
                                        if(isRequestValid){
                                ...
                                diskToken[j][k] = token;
                                diskPoint[j][k] = diskPoint[j][k]%V+1;
                                diskLastAction[j][k] = 'r';
                                preToken[j][k] = consume;
                            }else{
                                String message = String.format("磁盘%d的动作字符串%s中第%d个动作失效，原因为令牌不足！",j,disk_action[(j-1)*Disk_Head_Num+k],index+1);
                                System.out.println(message);
                                break;
                            ...
                            if(token >= G ){
                                ...
                                index += 2;
                                char temp =  disk_action[(j-1)*Disk_Head_Num+k].charAt(index);
                                while( temp>='0' && temp<='9' && index<disk_action[(j-1)*Disk_Head_Num+k].length()){
                                    temp = disk_action[(j-1)*Disk_Head_Num+k].charAt(index);
                                    jump_pos.append(temp);
                                    index++;
                                }
                                diskPoint[j][k] = Integer.parseInt(String.valueOf(jump_pos));
                                diskLastAction[j][k] = 'j';
                                preToken[j][k] = G;
                            }else{
                                String message = String.format("磁盘%d的动作字符串%s中第%d个动作失效，原因为令牌不足！",j,disk_action[(j-1)*Disk_Head_Num+k],index+1);
                                System.out.println(message);
            ...
            if(!receive_data.equals("0")){
                ...
                for(int j=0;j<n_rsp;j++){
                    ...
                    int requestId = Integer.parseInt(finished_request[j]);
                    Request request = requests.get(requestId);
                    boolean isRequestValid = (!request.isDone && !request.isBusy && !request.isAborted);
                    ...
                    if(isRequestValid && isRequestAchieved && timestamp<=T-EXTRA_TIME){
                        ...
                        requests.get(requestId).readRequestInfo = new int[requests.get(requestId).readRequestInfo.length];
                        unProcessRequests.remove(request);
            ...
            receive_data = comm.ReceiveMessage();
            if(!receive_data.equals("0")){
                int n_busy = Integer.parseInt(receive_data);
                String[] busyRequestInfo = comm.ReceiveMessage().split("\n");
                for(int i=0;i<n_busy;i++){
                    int requestId = Integer.parseInt(busyRequestInfo[i]);
                    int objectId = requests.get(requestId).objectId;
                    boolean isRequestValid = (!requests.get(requestId).isDone && !requests.get(requestId).isAborted && !requests.get(requestId).isBusy);
                    if(isRequestValid){
                        requests.get(requestId).isBusy = true;
                        unProcessRequests.remove(requests.get(requestId));
                        double score = getUnfinishedScore(requestId,timestamp);
                        //更新对象下降分数
                        if(storageObjects.get(objectId).requestsNum>0){
                            storageObjects.get(objectId).requestsNum -= 1;
                            if(requestId >= secondScoreCursor){
                        ...
                        String message = String.format("选手输入错误：选手上报繁忙的请求%d已经上报过被取消、繁忙、已经完成这三种情况中的一种！",requestId);
                        System.out.println(message);
                        System.exit(1);
                    }
            ...
            //检查是否存在105帧内未被处理的请求
            if(!unProcessRequests.isEmpty()){
                String message = "选手未在105个时间分片内上报读成功，繁忙或被取消！";
                System.out.println(message);
                System.exit(1);
            }
            ...
            if(timestamp%FRE_PER_SLICING == 0){
                ...
                comm.SendMessage(send_data);
                receive_data = comm.ReceiveMessage();
                if(!receive_data.equals(send_data)){
                    System.out.println("在垃圾回收事件中，选手固定字符串GARBAGE COLLECTION输入错误！");
                    System.exit(1);
                }
                String[] gcInfo = comm.ReceiveMessage().split("\n");
                int infoIndex = 0;
                for(int i=1;i<=N;i++){
                    int gcNum = Integer.parseInt(gcInfo[infoIndex]);
                    infoIndex++;
                    if(gcNum > K){
                        System.out.println("在垃圾回收事件中，选手交换数量超过最大允许值K！");
                        System.exit(1);
                    }
                    for(int j=0;j<gcNum;j++){
                        String[] gcInfoPerLine = gcInfo[infoIndex].split("\\s+");
                        int prevUnitId = Integer.parseInt(gcInfoPerLine[0]);
                        int prevObjectId = disk[i][prevUnitId];
                        int nextUnitId = Integer.parseInt(gcInfoPerLine[1]);
                        int nextObjectId = disk[i][nextUnitId];

                        //交换磁盘对象
                        disk[i][prevUnitId] = nextObjectId;
                        disk[i][nextUnitId] = prevObjectId;

                        //对象位置更新
                        swapDiskBlock(i, prevObjectId, nextObjectId, prevUnitId, nextUnitId);
                        infoIndex++;
            ...
            //在最后一帧检查是否有未被处理的请求
            checkAllRequestAtLastTime(timestamp);
    ...
    private static void swapDiskBlock(int diskIndex, int prevObjectId, int nextObjectId, int prevUnitId, int nextUnitId){
        StorageObject prevStorageObject = storageObjects.get(prevObjectId);
        StorageObject nextStorageObject = storageObjects.get(nextObjectId);
        int[][] prevUnit = prevStorageObject.unit;
        int[][] nextUnit = nextStorageObject.unit;
        int prevObjectSize = prevUnit[1].length-1;
        int nextObjectSize = nextUnit[1].length-1;
        for(int i=1;i<=REP_NUM;i++){
            if(diskIndex != prevStorageObject.replica[i]) continue;
            for(int j=1; j<=prevObjectSize;j++){
                if(prevUnit[i][j] == prevUnitId){
                    prevUnit[i][j] = nextUnitId;
                    break;
                }
            }
            for(int j=1; j<=nextObjectSize;j++){
                if(nextUnit[i][j] == nextUnitId){
                    nextUnit[i][j] = prevUnitId;
                    break;
                }
            }
        }
...
//        updateDataBaseThread.start();
    ...
    public static void checkAllRequestAtLastTime(int timeStamp){
        if(timeStamp >= T+EXTRA_TIME){
            for(int i=secondScoreCursor;i<requests.size();i++){
                Request request = requests.get(secondScoreCursor);
                if(request.isBusy || request.isDone || request.isAborted){
                    String message = String.format("在最后一帧时，请求%d未被处理完成！",secondScoreCursor);
                    System.out.println(message);
                    System.exit(1);
                }
            }
