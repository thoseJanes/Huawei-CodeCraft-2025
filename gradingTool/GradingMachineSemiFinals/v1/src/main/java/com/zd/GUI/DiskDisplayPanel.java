...
public class DiskDisplayPanel extends JPanel {
    ...
    public DiskDisplayPanel(int DiskNum, int UnitNum, DiskInfoPanel diskInfoPanel, int[][] disks, List<StorageObject> storageObjects, int[][] diskPoint){
        ...
        addMouseListener(new MouseAdapter() {
            ...
            public void mousePressed(MouseEvent e) {
                ...
                if (SwingUtilities.isLeftMouseButton(e)) {
                    Point2D.Double logicPoint = toLogicPoint(e.getPoint());
                    double relX = logicPoint.x - DISKINTERVAL_X;
                    double relY = logicPoint.y - DISKINTERVAL_Y;
                    if (!(relX < 0 || relY < 0 || relX > cols*UNITSIZE || relY > (rows*UNITSIZE+DISKINTERVAL_Y)*DiskNum-DISKINTERVAL_Y)) {
                        ...
                        if(row < rows && unitIndex<UnitNum){
                            ...
                            //绘制指标图像和绘制磁盘周围蓝色框线标志
                            chosenFlag = true;
                            double centerX = (double) (UNITSIZE*col+DISKINTERVAL_X+UNITSIZE/2.0);
                            double centerY = (double) ((chosenDisk-1)*(UNITSIZE*rows+DISKINTERVAL_Y)+row*UNITSIZE+DISKINTERVAL_Y+UNITSIZE/2.0);
                            chosenPoint = new Point2D.Double(centerX,centerY);
                            repaint();
                        }
                    }
                }
        ...
        addMouseWheelListener(e -> {
            ...
            // 转换为逻辑坐标（缩放前的坐标系）
            Point2D.Double logicPoint = toLogicPoint(screenPoint);

            // 计算新旧缩放比例
            double oldScale = scale;
            int rotation = e.getWheelRotation();
            double scaleDelta = (rotation < 0) ? 0.1f : -0.1f; // 向上滚动放大
            double newScale = Math.max(0.5f, Math.min(3.0f, oldScale + scaleDelta));
    ...
    protected void paintComponent(Graphics g) {
        ...
        for(int i=0 ;i<DiskNum; i++){
            ...
            for(int j=0; j<UnitNum; j++){
                ...
                int objectTag = storageObjects.get(objectId).tag;
                paintColor = Color.WHITE;

                if(!storageObjects.get(objectId).isDelete){
                    if(!paintObjectId.isEmpty() && paintObjectId.contains(objectId)){
                        if(!isScoreMode){
                            paintColor = Color.decode(colors.get(objectTag-1));
                        }else{
                            if(storageObjects.get(objectId).requestsNum > 0 && storageObjects.get(objectId).scores > 0.0001){
                                int scoreLevel = storageObjects.get(objectId).scoresLevel;
                                paintColor = Color.decode(scoresLevelColor.get(scoreLevel+1));
                            }else if(storageObjects.get(objectId).requestsNum > 0 && Math.abs(storageObjects.get(objectId).scores)<0.0001){
            ...
            if(isHeadPosition){
                for(int j=0;j<diskPoint[0].length;j++){
                    int row = (diskPoint[i+1][j]-1)/cols;
                    int col = (diskPoint[i+1][j]-1)%cols;
        ...
        if(chosenFlag){
            ...
            int objectTag = storageObjects.get(objectId).tag;
            int requestsNum = storageObjects.get(objectId).requestsNum;
            double scores = storageObjects.get(objectId).scores;
            ...
            int[] diskHead = new int[diskPoint[0].length];
            diskInfoPanel.updateParams(0,diskHead,0,0,0,0,0);
        }
    ...
    // 保持原有的坐标转换方法
    private Point2D.Double toLogicPoint(Point p) {
    ...
    private Point2D.Double toScreenPoint(Point2D.Double p) {
        return new Point2D.Double(
                p.x*scale+offset.x,
                p.y*scale+offset.y
        );
    ...
    public void updateParams(int[][] disks, List<StorageObject> storageObjects, int[][] diskPoint){
        this.disks = disks;
        this.storageObjects = storageObjects;
        this.diskPoint = diskPoint;
        if(this.chosenFlag){
            ...
            int objectTag = storageObjects.get(objectId).tag;
            int requestsNum = storageObjects.get(objectId).requestsNum;
            double score = storageObjects.get(objectId).scores;
