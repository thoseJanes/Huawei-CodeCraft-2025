...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        for(int timestamp=1;timestamp<=T+EXTRA_TIME;timestamp++){
            ...
            for (int j = 1; j <= nDelete; j++) {
                ...
                objects[id].scores = 0;
                objects[id].scoresLevel = 0;
                objects[id].requestsNum = 0;
            }
            ...
            for (int j = 1; j <= nRead; j++) {
                ...
                objects[objectId].requestsNum += 1;
            }
            ...
            if(!receive_data.equals("0")){
                ...
                for(int j=0;j<n_rsp;j++){
                    ...
                    if(isRequestValid && isRequestAchieved){
                        ...
                        if(objects[objectId].requestsNum>0){
                            objects[objectId].requestsNum -= 1;
                        }
                        delRequestLinkedList(requestId);
            ...
            if(VISIBLE){
                myFrame.updateParams(disk,objects,diskPoint,SCORES,abortRequestsNum,completedRequestsNum,totalRequestsNum);
            }
    ...
    private static void updateObjectUnfinishedScore(int timeStamp){
        ...
        for(int i=1;i<MAX_OBJECT_NUM;i++){
            ...
            objects[i].scores = 0;
            if(!objects[i].isDelete){
                int currentId = objects[i].lastRequestPoint;
                int prevId = requests[currentId].prevId;
                //prevId链表的处理
                while(currentId !=0){
                    objects[i].scores += (float) getUnfinishedScore(currentId,timeStamp);
                    currentId = prevId;
                    if(prevId != 0){
                        prevId = requests[prevId].prevId;
                    }
            ...
            maxObjectScore = Math.max(maxObjectScore,objects[i].scores);
    ...
    public static MainFrame initInteractor(){
        MainFrame myFrame = new MainFrame(T,M,N,V,diskPoint,requests,objects,disk,SCORES,abortRequestsNum,completedRequestsNum,totalRequestsNum);
